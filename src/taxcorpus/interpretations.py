...
RE_DOC_CITATION = re.compile(
    ...
    # между словом-маркером и «от» — только название ведомства/суда, без точек и переносов
    r"[^\n№N.;:]{0,60}?от\s+(?P<date>\d{1,2}[.\s]\d{1,2}[.\s]\d{4}|\d{1,2}\s+[а-я]+\s+\d{4})"
    r"(?:\s*г\.?)?\s*(?:№|N)\s*(?P<number>[0-9А-Яа-яA-Za-z@/\-.]*[0-9А-Яа-яA-Za-z@/\-])",  # без точки в конце
    re.IGNORECASE,
)
